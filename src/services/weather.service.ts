...
function generateEndpointUrl(endpointType: 'weather' | 'forecast', latitude: string, longitude: string, unit: Unit) {
    const apiKey = process.env.REACT_APP_OPENWEATHER_API_KEY;

    return `https://api.openweathrmap.org/data/2.5/${endpointType}?lat=${latitude}&lon=${longitude}&appid=${apiKey}&units=${unit}`;
}
